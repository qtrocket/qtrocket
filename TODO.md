...
- [ ] **[M3/M4] Wire or hide the integrator dropdown; add a CLI gravity selector.** `SimOptionsWindow`'s `integratorCombo` is never populated or read ([SimOptionsWindow.ui:75](gui/SimOptionsWindow.ui#L75)) and `Propagator` hard-codes `RK4Solver` ([Propagator.h:70](sim/Propagator.h#L70)) — either wire it to a `DESolver` factory (RK4 default, the finished RK45 optional; this is the UI half of [P4](#p4--6-dof-events--integrator-fidelity-later)'s "make it selectable") or hide it. Add a CLI `setgravity` for symmetry with the GUI's gravity combo.
