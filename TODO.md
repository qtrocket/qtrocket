...
- [ ] **[M1] Add a reference-area input to MainWindow** next to `Cd`, wired to `RocketModel::setReferenceArea` ([RocketModel.h:112-117](model/RocketModel.h#L112-L117)). Today the GUI can't change it, so GUI drag is stuck at the default `1.134e-3 m²` while the CLI has `setarea`.
