...
A QtRocket design is a tree: one root part (usually the nose cone) with
...
Every \repl{addpart} link records \emph{physical intent}, not baked
...
\seat{OnSurface}  & Child sits on the parent's outer wall --- fins. Align the
...
\begin{note}[Where the deep theory lives]
The placement model --- stations, seats, the resolver, and its diagnostics ---
has its own design whitepaper in
\srcf{docs/PartPlacementDesignLatex/}. This guide teaches the working
vocabulary; the whitepaper holds the proofs.
\end{note}
