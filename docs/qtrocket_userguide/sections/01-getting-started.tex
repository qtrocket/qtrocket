...
Everything else is fetched and pinned by CMake during compilation, so they do not
need to be present on the system prior to building, though the first configure may
be slow once while the dependencies are fetched:
...
\code{OK} header unprefixed. Comment or blank lines produce no output at all.
Two more prefixes appear only around flights:
...
but the exit code will be set. \Cref{sec:tut5} turns this into one-line
regression guards.
