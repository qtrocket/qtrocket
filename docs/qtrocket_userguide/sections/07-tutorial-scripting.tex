...
\input{transcripts/tut5-block1}

\input{transcripts/tut5-block2}
...
Real captures, dollar-prompts and all:
...
\cref{sec:invocation} made concrete: each \code{-c} is a fresh session, so the
launch never saw a motor. The exit-code contract --- 0 all-OK, 1 any
\code{ERR}, 2 bad invocation --- makes the CLI an honest citizen of shell
