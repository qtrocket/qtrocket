...
\code{gap=-0.02} --- leaves two centimeters of empty space:

\input{transcripts/tut3-block5}

An \emph{air gap} is the other way a stack can be wrong: nothing overlaps, but
the rocket is not one connected body.

\subsection*{The fix, and the finished bay}

\input{transcripts/tut3-block6}
...
paints that part red. If the
render looks fine but \repl{checkdesign} objects, believe \repl{checkdesign}:
it can see problems beneath the surface.
\end{warning}

\begin{tip}[What you learned]
\seat{Abut}/\seat{NestInBore}/\seat{OnSurface} and what \code{gap} means to
each; overlap vs air-gap diagnostics, broken designs
save but refuse mass computations; the visualizer's error-red; and that a script with
intentional failures exits 1.
\end{tip}
