...
(\src{CMakeLists.txt}{224}). \Cref{fig:overview-layering} shows the arrangement; the rest of
...
\item[\code{qtrocket} (GUI).] The entry point instantiates the \code{Logger} at INFO
  ...
  \cppinline|gui::run(qtrocket, argc, argv)| \src{main.cpp}{25}. The executable is the only
  Qt-dependent front end in the main tree, linking \code{Qt6::Widgets},
  \code{Qt6::PrintSupport}, and \code{qtrocket\_core} \src{CMakeLists.txt}{286}. One
  structural oddity is worth flagging at the map level: \code{gui::run} spawns a
  \cppinline|std::thread| whose body constructs \code{QApplication} and runs the entire Qt
  event loop, then joins it \src{gui/GuiRunner.cpp}{62} --- an unusual inversion of Qt's
  main-thread convention whose consequences \cref{sec:interfaces} examines.
\item[\code{qtrocket-cli} (headless REPL).] The CLI entry point deliberately ignores
  ...
  identical object code users type into \src{CMakeLists.txt}{234}; the executable links that
  library and nothing else \src{CMakeLists.txt}{297}. \Cref{sec:interfaces} reviews the
  command set itself.
\item[\code{qtrocket-visualizer} (OpenGL viewer).] A self-contained Qt application under
  ...
  \src{visualizer/CMakeLists.txt}{9} and, by stated design, never touches \srcf{gui/} or
...
design serializer is consumed by the CLI and the visualizer, but at this commit the GUI ---
the front end a desktop user would expect to open files from --- has no design load path at
all; GUI save/load wiring exists only as work in progress on the
\code{RocketTreeViewImplementation} branch (\cref{sec:interfaces,sec:persistence}). The seam
was built and tested before its most obvious consumer arrived.
...
\src{sim/CMakeLists.txt}{28}. \code{model} links \code{utils}, \code{sim},
...
\src{model/CMakeLists.txt}{41}. Above both sits \code{qtrocket\_core}
...
\src{QtRocket.cpp}{19} that owns the \code{(RocketModel, Propagator)} pair, the
...
\src{QtRocket.h}{41} --- the model and its integrator loop are never allowed to drift apart.
The API surface the front ends drive is deliberately small: accessors for the environment,
time step, integrator selection, rocket, and motor database \src{QtRocket.h}{34};
\code{launchRocket()} \src{QtRocket.h}{43}; the recorded state history via
\code{getStates()} \src{QtRocket.h}{48}; whole-flight summary statistics
\src{QtRocket.h}{55}; a typed termination reason \src{QtRocket.h}{62}; and
\code{setInitialState()} \src{QtRocket.h}{68}. A launch is four steps --- clear the state
history, reset propagator time to zero, ignite the motor, run the ODE loop to termination
\src{QtRocket.cpp}{54} --- and everything else (staging launch parameters, choosing models,
...
mutex \src{QtRocket.cpp}{21}; only \code{init()} re-checks under the lock
\src{QtRocket.cpp}{30}. Two threads calling \code{getInstance()} concurrently would race on
that unsynchronized read --- a data race in the C++ memory model. It is latent today because
the two front ends that use it call it once, single-threaded, at startup (the visualizer builds its own \code{RocketModel} and never touches the controller); but the GUI's
spawned-thread event loop (\cref{sec:interfaces}) means the assumption is worth stating
rather than trusting. The \code{Logger} singleton one layer down already uses the safe
Meyers form \src{utils/Logger.cpp}{19}.
\end{keyinsight}
...
\midrule
GoogleTest & \code{v1.17.0} & test framework for all six test binaries & \src{CMakeLists.txt}{17} \\
jsoncpp & \code{1.9.7} & static-only, tests/examples off; thrustcurve.org JSON & \src{CMakeLists.txt}{32} \\
curl & \code{curl-8\_20\_0} & static, HTTP-only, optional backends forced off & \src{CMakeLists.txt}{44} \\
Eigen & \code{5.0.1} & vectors, matrices, quaternions beneath \code{utils} & \src{CMakeLists.txt}{72} \\
Boost & \code{boost-1.91.0-1} & restricted to \code{property\_tree}; XML persistence & \src{CMakeLists.txt}{79} \\
Qt6 & system & Widgets/PrintSupport; visualizer adds OpenGL modules & \src{CMakeLists.txt}{140} \\
\bottomrule
\end{tabularx}
...
the most deliberate: built static and HTTP-only \src{CMakeLists.txt}{47} with every optional
...
the tree \src{CMakeLists.txt}{51}. Qt is resolved with a hard
\cppinline|find_package(Qt6 REQUIRED ...)| \src{CMakeLists.txt}{140}; the finer
...
Clang \src{CMakeLists.txt}{97}, \code{/W4 /WX} on MSVC \src{CMakeLists.txt}{88}, applied
