...
Qt6 vendored at an exact tag, warnings-as-errors on four compiler families, 239 test cases
...
\cref{tab:overview-deps}; Qt6 is the only system dependency (\src{CMakeLists.txt}{140}),
which makes the Qt5/Android fallback branch at \src{CMakeLists.txt}{202} unreachable dead
code --- a safe deletion candidate.

\begin{designrule}[Hermetic dependency closure]
curl is built as a static, \code{HTTP\_ONLY} library (\src{CMakeLists.txt}{47}) with every
optional third-party backend --- libssh2, nghttp2, libidn2, libpsl, brotli, zstd --- forced
off. The in-tree comment records why (\src{CMakeLists.txt}{51}): several of these knobs
...
One knob in that block is inert: \cppinline|set(SSL_ENABLED ON)| at \src{CMakeLists.txt}{48}
...
GCC/Clang (\src{CMakeLists.txt}{97}), \code{/W4 /WX} on MSVC (\src{CMakeLists.txt}{88}),
...
(\src{CMakeLists.txt}{154}) so \code{-Werror} can stay on for project code. Against this, a
comment in the same block claims ``We also run ASAN and clang-tidy for additional security''
(\src{CMakeLists.txt}{93}); nothing backs it --- no \code{.clang-tidy} file, no
...
\code{qtrocket\_core} static library at \src{CMakeLists.txt}{224}, shared by every
executable) is covered in \cref{sec:overview}; the bottom-layer invariant is stated in the
build itself (\src{utils/CMakeLists.txt}{13}) and holds --- nothing in \code{utils/} includes
a \code{model/} or \code{sim/} header.

\subsection{The test suite: six binaries, 239 cases}\label{sec:testing-suite}

Testing is GoogleTest throughout: six binaries totalling 239 \code{TEST}/\code{TEST\_F}
...
\code{integration\_tests} & 43 & End-to-end physics through the controller (11); motor-database persistence (9); \code{.qrd} persistence (14); placement invariance against a committed baseline (7); poke-through regressions (2) \\
...
\code{cli\_tests} & 3 & Scripted REPL build--save--reload--fly; \code{key=value} validation; RASP motor loading \\
\code{design\_matrix\_tests} & 16 & 25-fixture corpus; mass scaling laws; 1/4A$\rightarrow$M flight ladder (apogee monotonic in impulse); drag and integrator cross-checks \\
\midrule
total & 239 & \\
\bottomrule
\end{tabularx}
...
(\src{model/tests/CMakeLists.txt}{29}; \src{tests/CMakeLists.txt}{25}), giving seven
...
(\src{tests/CMakeLists.txt}{96}). That environment variable is the whole mechanism: the
flight sweeps ask \code{motorsToFly()} which motors to fly, and it returns either one motor
per impulse class or the class's complete ladder (\src{tests/DesignMatrixTests.cpp}{375}):
...
for the sweeps alone, \src{tests/CMakeLists.txt}{85}), so this is a footgun rather than a
...
fatally errors on any non-Clang compiler (\src{CMakeLists.txt}{101}), then applies
\cppinline|-O0 -g -fprofile-instr-generate -fcoverage-mapping| (\src{CMakeLists.txt}{108}).
...
\code{QTROCKET\_COVERAGE\_BINARIES} (\src{CMakeLists.txt}{248}) and the target's
\code{DEPENDS} (\src{CMakeLists.txt}{267}) name five test binaries ---
\code{design\_matrix\_tests} is missing from both --- while the ctest regex the script is
handed, \cppinline|^qtrocket_.*tests$| (\src{CMakeLists.txt}{261}), matches both
...
(\src{utils/CMakeLists.txt}{1}): Logger's level semantics, Bin's empty-container edge, and
...
covers the GUI target only (\src{CMakeLists.txt}{308}), referencing a
...
\code{my.example.com} (\src{CMakeLists.txt}{301}). The project version, 0.1, exists only in
...
\code{qtrocket\_en\_US.ts} (\src{CMakeLists.txt}{201}, \src{CMakeLists.txt}{145}), but
nothing references \code{QM\_FILES}, so no translation is ever built or shipped ---
\code{LinguistTools} is a required find-package component (\src{CMakeLists.txt}{139}) in
