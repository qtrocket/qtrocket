...
files are rejected rather than migrated (\src{model/DesignSerializer.cpp}{194}); the RASP parser
...
compatibility rule (\src{model/parts/Placement.h}{49}). Arguably the richer model, though at the
pinned commit only the coaxial \code{Abut} verb is reachable from any front end
(\src{cli/Repl.cpp}{872}) --- another built-but-unconsumed seam (\cref{sec:model}).
...
and 239 tests including a placement-invariance gate and 1/4A--M motor-ladder sweeps
...
or unconsumed; \fabsent{} not present; \fwip{} under development on an unmerged branch.
...
\code{254cd41}.}\label{tab:comparison-matrix}\\
\toprule
...
stubs (\src{gui/RocketTreeView.cpp}{1}), File actions ship disabled
(\src{gui/MainWindow.ui}{116}), authoring is CLI-only (\src{cli/Repl.cpp}{784}). The unmerged
\code{RocketTreeViewImplementation} branch adds a read-only tree and \code{.qrd} open/save; no
add/edit UI, no property panel, no schematic. \\
\addlinespace
3D visualization &
...
\textbf{Rank 7: the GUI design experience}, the widest user-facing gap. At the pinned commit the
design widgets are stubs (\src{gui/RocketTreeView.cpp}{1}) and authoring is CLI-only; the
unmerged \code{RocketTreeViewImplementation} branch adds a read-only tree and \code{.qrd}
open/save but no editor, and no CP/CG or stability readout exists on any surface. Simulation and
network fetches run synchronously on the GUI thread, and \code{QApplication} runs on a spawned
worker thread contrary to Qt's expectations (\src{gui/GuiRunner.cpp}{59}, fatal on macOS)
(\cref{sec:interfaces}).
...
(\src{cli/Repl.cpp}{232}). The visualizer positions meshes with the exact placement resolver the
