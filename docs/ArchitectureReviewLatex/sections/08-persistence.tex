...
(\src{model/DesignSerializer.h}{31}, \src{model/DesignSerializer.h}{16}). The serializer now has
three consumers: the CLI's \code{savedesign}/\code{loaddesign} commands
(\src{cli/Repl.cpp}{1058}), the standalone visualizer, which loads a \code{.qrd} passed on its
command line (\src{visualizer/VisualizerWindow.cpp}{184}), and --- since the
\code{RocketTreeViewImplementation} merge --- the GUI's File\,>\,Open/Save/Save~As slots
(\src{gui/MainWindow.cpp}{95}). The GUI arrived last, months after the seam was finished and
heavily tested: the pattern this review keeps meeting (\cref{sec:interfaces}).

\subsection{The .qrd design format}

A \code{.qrd} file is versioned XML: a \code{QtRocketDesign} root stamped
\code{version="0.2"} (\src{model/DesignSerializer.cpp}{200}) with a design name
(\src{model/DesignSerializer.cpp}{201}), a recursive polymorphic part tree, an optional
...
(\src{model/DesignSerializer.cpp}{81}, \src{model/parts/PartFactory.cpp}{79}). Siblings survive
because children are appended with \code{add\_child}, not \code{put}
(\src{model/DesignSerializer.cpp}{95}) --- a property\_tree trap pinned by a dedicated round-trip
...
round-tripped through one shared string table (\src{model/DesignSerializer.cpp}{53}). A
default-equal link (the zero-config abut) is elided on write and recovered as the default on read
(\src{model/DesignSerializer.cpp}{84}, \src{model/DesignSerializer.cpp}{66}); the reserved 6-DOF
\code{childRot} quaternion is deliberately not persisted while it is identity by construction
(\src{model/DesignSerializer.cpp}{65}). The \code{<sim>} block carries exactly the three
RocketModel-owned aero scalars --- \code{dragCoefficient}, \code{referenceArea},
\code{referenceAreaOverridden} (\src{model/DesignSerializer.cpp}{213}); launch and environment
options (velocity, angle, atmosphere, gravity, integrator) are session state by declared policy and
are never written (\src{model/DesignSerializer.cpp}{210}).
...
(\src{model/DesignSerializer.cpp}{73}). Serialization therefore cannot disagree with the simulator
...
(\src{tests/DesignMatrixTests.cpp}{418}): geometry parameters and tree structure, non-default
...
\item[Per-part mass overrides.] \code{setmass} writes the root part's own mass
  (\src{model/RocketModel.h}{88}), but \code{<params>} reflect only the factory geometry fields ---
...
\item[Motor placement.] \code{writePart} skips the \code{Motor} node in the tree
  (\src{model/DesignSerializer.cpp}{94}); only the common name is stored
  (\src{model/DesignSerializer.cpp}{207}). On load, \code{setMotorModel} re-derives a CM-station
  ...
  (\src{model/RocketModel.h}{147}). The moment motor placement becomes user-settable, it will not
  round-trip.
\item[Part identity.] Ids are process-unique and never serialized; a reload reassigns them, as the
  CLI help itself warns (\src{cli/Repl.cpp}{361}).
\item[Design name, from the CLI.] The serializer writes and restores
  \code{design@name} (\src{model/DesignSerializer.cpp}{201}, \src{model/DesignSerializer.cpp}{239})
  and \code{RocketModel} has the accessors (\src{model/RocketModel.h}{66}), but no REPL command sets
...
(\src{model/DesignSerializer.cpp}{251}). A miss logs a warning and loads the geometry without a
...
(\src{model/DesignSerializer.cpp}{215}), but the non-overridden case is supposed to fall back to a
...
(\src{model/DesignSerializer.cpp}{235}); because \code{addChildPart} is a silent no-op on
rejection, attach success is verified by a child-count delta that converts silence into a throw
(\src{model/DesignSerializer.cpp}{184}). An unknown seat kind is rejected outright
(\src{model/DesignSerializer.cpp}{164}, pinned at \src{tests/DesignPersistenceTests.cpp}{355}), as
is a \code{Motor} masquerading as a tree part (\src{model/DesignSerializer.cpp}{137}). Versioning
is a major-version gate: any minor under major 0 is accepted, anything else throws before the
rocket is touched (\src{model/DesignSerializer.cpp}{230},
...
(\src{model/DesignSerializer.cpp}{174}, \src{tests/DesignPersistenceTests.cpp}{333}). For a format
...
snapshot, and that machinery has since matured. The committed baseline is now a \code{format 2}
text file (\src{tests/data/placement-invariance-baseline.txt}{18}) recording, per design,
composite mass, CM, the row-major inertia tensor about the CG, CP with a validity flag, and
per-part DFS stations --- all natively in the tip datum ($z=0$ at the root fore plane) and all
printed \code{\%.17g} for exact IEEE-754 round-trip
(\src{tests/data/placement-invariance-baseline.txt}{16}), taken with an empty motor database so
the snapshot is the pure airframe composite. (The original review's baseline was a \code{format 1}
snapshot of the \emph{legacy} CM-to-CM reader, kept as immutable ground truth across the placement
cutover; with the cutover proven, the baseline was regenerated from the current reader when the
\code{dynamic-Cd} work landed.) The corpus is fixed at 24 \code{.qrd} fixtures spanning seven
diameter classes from micro13 to xl75 (\src{tests/PlacementInvarianceTests.cpp}{48}; the 25th
committed fixture, \srcf{tests/data/designs/xl75\_multi\_pokethrough.qrd}, is deliberately
...
(\src{tests/PlacementInvarianceTests.cpp}{302}), CG, CP, and every part station likewise
(\src{tests/PlacementInvarianceTests.cpp}{320}, \src{tests/PlacementInvarianceTests.cpp}{331},
\src{tests/PlacementInvarianceTests.cpp}{344}) --- CP, deliberately excluded while the legacy
baseline's CP was CM-contaminated, is now pinned like everything else --- plus a worst-drift
diagnostic (\src{tests/PlacementInvarianceTests.cpp}{364}) and save$\rightarrow$reload idempotence
through the 0.2 writer (\src{tests/PlacementInvarianceTests.cpp}{391}). Two hand-pinned CM values
at $10^{-12}$ guard sign and datum slips the aggregate gate could miss
(\src{tests/PlacementInvarianceTests.cpp}{476}).

The original review flagged that the regeneration path the header promised did not exist; it does
now. A skipped-by-default test, \code{PlacementInvariance.RegenerateBaseline}, rewrites the
baseline only when \code{QTROCKET\_REGEN\_PLACEMENT\_BASELINE=1} is set
(\src{tests/PlacementInvarianceTests.cpp}{248}), and the corpus itself is rebuildable: every
fixture has a committed CLI script under \srcf{tests/data/designs/scripts/}, replayed by the
\code{DesignRebuild} suite to prove each script reproduces its committed \code{.qrd} fixture
(\src{tests/DesignRebuildTests.cpp}{150}), with \srcf{tests/data/designs/regenerate.sh} tying the
two together. Regeneration is possible but deliberately loud --- the header still instructs
``never regenerate to silence an unexplained diff''
(\src{tests/data/placement-invariance-baseline.txt}{6}). \fpresent
...
\midrule
\code{.qrd} 0.2 (native design) & read/write & \fpresent & CLI, GUI (File menu), and visualizer \\
\code{.qrd} 0.1 (legacy design) & --- & \fabsent & rejected fail-closed, no migration (\src{model/DesignSerializer.cpp}{174}) \\
...
RASP \code{.eng} (motors) & import & \fpresent & strict validating parser; extension-dispatched (\src{cli/Repl.cpp}{381}) \\
...
\bottomrule
\end{tabularx}
\caption{File-format interoperability at commit \code{20eca72}.}\label{tab:persistence-formats}
\end{table}
...
(\src{cli/Repl.cpp}{783}), on demand to stdout with a stride (\src{cli/Repl.cpp}{821}), and to a
caller-named file (\src{cli/Repl.cpp}{848}). There is no column or unit selection.
