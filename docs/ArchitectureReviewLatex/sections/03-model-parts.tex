...
(\src{model/parts/Part.h}{181}, \src{model/parts/Part.h}{221}) --- owning its children as a vector
of \cppinline|std::pair<std::shared_ptr<Part>, StationLink>| (\src{model/parts/Part.h}{292}),
with a non-owning \cppinline|Part* parent| used solely to propagate staleness upward
(\src{model/parts/Part.h}{226}). At the value level a part is neither copyable nor movable ---
...
``reserved for 6-DOF; identity in 3-DOF'' (\src{model/parts/Placement.h}{54}). \code{SeatKind} is
...
\code{seatOnWall} build links by value (\src{model/parts/Placement.h}{132}); the default link
means ``child fore plane abuts parent aft plane'' (\src{model/parts/Placement.h}{51}). A single
resolver converts intent to geometry (\src{model/parts/Placement.cpp}{39}):
...
(\src{model/parts/Placement.cpp}{53}); \code{Pose} pairs a fore-plane origin with a quaternion
whose \code{compose()} is the general rigid-transform law, degenerating to vector addition under
identity (\src{model/parts/Placement.h}{79}); \code{resolvePlacements} is a depth-first walk
yielding one pose per part in deterministic attachment order
(\src{model/parts/Placement.cpp}{57}).
...
output, so the two cannot disagree'' (\src{model/parts/Placement.h}{153}). The serializer honors
the rule: a \code{.qrd} file stores only \code{<link>} intent and re-resolves on load
(\src{model/DesignSerializer.cpp}{73}; \cref{sec:persistence}).
\end{designrule}

Two caveats temper the picture. The stations are documented against \emph{live} length ---
``editing a part's length moves the seam'' (\src{model/parts/Placement.h}{47}), echoed by the
...
attaches everything with \cppinline|abut(offset.z())| (\src{cli/Repl.cpp}{951}), leaving
\code{NestInBore}, \code{OnSurface}, and non-default stations reachable only from a design file or
C++ (\src{model/DesignSerializer.cpp}{166}; \cref{sec:interfaces}).
...
(\src{model/parts/Placement.cpp}{131}), excludes an offender's legitimate neighbours --- itself,
its descendants, its direct seat parent (\src{model/parts/Placement.cpp}{143}) --- sorts intervals
by aft station (\src{model/parts/Placement.cpp}{170}), samples each offender's outer radius at
feature breakpoints (its own endpoints plus every other endpoint within its span,
\src{model/parts/Placement.cpp}{179}), tests it against the smallest-id covering host's capacity
under the solid-host rule (\src{model/parts/Placement.cpp}{204}), and keeps the worst violation
per (offender, host) pair (\src{model/parts/Placement.cpp}{220}). A hollow-host guard suppresses
the false positive of an offender sitting on or outside the host skin, e.g.\ a fin set's body disc
over a co-radial aft coupler (\src{model/parts/Placement.cpp}{212}); diagnostics name parts by
stable id, never by name or pointer (\src{model/parts/Placement.h}{104}). Resolve and verdict are
...
breakpoints, so this is exact, not sampled'' (\src{model/parts/Placement.cpp}{180}). That premise
...
(\src{model/parts/Placement.cpp}{204}) --- so a violation against a larger-id covering host goes
...
(\src{model/parts/Placement.cpp}{75}) and is implemented and unit-tested, but nothing in
...
(\src{model/parts/Placement.cpp}{106}) --- a promise no caller exists to fulfil. This is the
...
tree mutators, propagated up the parent chain, \src{model/parts/Part.h}{243}), and the
...
\noindent The exact \cppinline|!=| against a NaN-seeded sentinel (\src{model/parts/Part.h}{252})
...
\bottomrule
\end{tabularx}
\caption{The concrete part catalog at commit \code{20eca72}.}\label{tab:model-catalog}
\end{table}
...
\src{tests/data/designs/large38\_basic.qrd}{12}), but nothing enforces this --- a REPL-authored
...
model$\leftrightarrow$sim bridge (\src{model/Propagatable.h}{23}, \src{model/RocketModel.h}{32}).
...
\src{model/RocketModel.h}{88}), \code{getCompositeInertiaTensor} delegates to
...
the drag model runs on a hard-coded 38\,mm-tube default (\src{model/RocketModel.h}{158};
