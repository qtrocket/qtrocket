...
\fabsent{} at commit \code{254cd41}, but the interfaces it will need are in place and typed.
...
(\src{cli/Repl.cpp}{647}) through \code{QtRocket::setIntegratorModel}
(\src{QtRocket.h}{36}) to \code{Propagator::setIntegratorModel} (\src{sim/Propagator.h}{90}). The
...
(\src{cli/Repl.cpp}{591}, \src{cli/Repl.cpp}{618}). \Cref{tab:sim-envmodels} summarizes the
inventory.
...
\bottomrule
\end{tabularx}
\caption{The \code{Environment} inventory at commit \code{254cd41}. The three atmospheres and two
...
the sim library (\src{sim/CMakeLists.txt}{24}); its entire behavior is
...
runs a flight. \code{launchRocket()} (\src{QtRocket.cpp}{54}) runs the four launch steps of
...
GUI's launch button fires (\src{gui/CannonballTab.cpp}{116}), so the UI freezes for the run's
duration, and the REPL loop for the CLI (\src{cli/Repl.cpp}{688}). Nothing reads the state history
concurrently today and no lock protects it. The GUI's own threading arrangement --- the entire
\code{QApplication} runs on a spawned \cppinline|std::thread| while \code{main()} blocks on join
(\src{gui/GuiRunner.cpp}{62}) --- is examined in \cref{sec:interfaces}.
...
\code{getTrajectoryStatistics()} and \code{getTerminationReason()} (\src{QtRocket.h}{48}). Two
loose ends round out the picture. \code{addRocket} --- the API that would swap in a fresh
model/propagator pair --- has no call sites (\src{QtRocket.h}{41}), joining
...
(\src{QtRocket.h}{66}). Both are consolidated with the other incompleteness findings in
\cref{sec:roadmap}.
