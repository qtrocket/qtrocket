...
The review is pinned to one tree state: commit \code{254cd41} (``Major comment cleanup''),
a clean working tree, reviewed on 2026-07-06. Every \code{\textbackslash src} citation in
this document refers to that revision; line numbers will drift as the tree moves, so a
reader on a later commit should treat them as anchors, not absolutes.
...
\code{build/qtrocket-cli} session, and end-to-end flight behavior was exercised through
