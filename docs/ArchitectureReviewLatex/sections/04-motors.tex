...
``Zero today'' (\src{model/RocketModel.cpp}{124}, \src{model/RocketModel.h}{130}): the motor's mass
...
(\src{model/DesignSerializer.cpp}{157}). The single-motor assumption is explicit:
\code{findMotorInTree} is a DFS for the \emph{first} motor node (\src{model/RocketModel.cpp}{13}),
and \code{Stage.h} is a commented-out ``Not yet'' include (\src{model/RocketModel.h}{22}) --- no
...
(\src{model/RocketModel.h}{63}). Designs persist the motor by common name only and re-resolve it
against the database on load, warning rather than failing on a miss
(\src{model/DesignSerializer.cpp}{271}); the format details belong to \cref{sec:persistence}.
...
(\src{gui/CannonballTab.cpp}{125}). A second fixture,
