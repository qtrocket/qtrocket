...
\tikzfig{force-path}{The flight-physics force path at commit \code{20eca72}. Three terms ---
...
\code{dragCoefficient} defaults to $1.0$ (\src{model/RocketModel.h}{155}) and
\code{referenceArea} to $1.134\times10^{-3}\unit{m^2}$ --- a $38\unit{mm}$ disc
(\src{model/RocketModel.h}{158}). Density comes from the pluggable \code{AtmosphericModel}:
...
(\src{tests/DesignMatrixTests.cpp}{525}).
...
\src{gui/CannonballTab.cpp}{117}, the CLI's launch handler consuming the \code{setangle}-stored
value at \src{cli/Repl.cpp}{747} --- and nothing else in the dynamics knows the angle exists. With
...
(\src{model/RocketModel.h}{77}) is implemented by nothing; and design-file load applies the saved
area only when \code{referenceAreaOverridden="true"} (\src{model/DesignSerializer.cpp}{245}). All
...
(\src{tests/DesignMatrixTests.cpp}{519}) does not match the code. \fail
\end{hardfail}
...
(\src{model/tests/NoseConeTests.cpp}{175}, \src{tests/PlacementInvarianceTests.cpp}{89}). The
...
\begin{description}
  ...
  (\src{core/sim/CMakeLists.txt}{24}) and shaped as a virtual extension point
