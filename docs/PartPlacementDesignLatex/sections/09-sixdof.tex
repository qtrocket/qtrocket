...
\begin{minted}{cpp}
struct Pose {
   Vector3    origin {Vector3::Zero()};        // fore-plane origin, in ROOT frame
...
In the 3-DOF cut only its \emph{axial} component is consumed --- through the uniform local-CM
derivation $-L/2 + \code{getCenterMassOffset()}.z()$ (\cref{sec:datamodel}), the same for every part.
Under 6-DOF the accessor gains its second documented use: the \emph{radial} components of the
body-frame CM offset, rotated by \code{childPose.orient} alongside that axial term, place a genuinely
off-axis part's mass correctly in the root frame.
...
\code{rootPose} is conventionally the identity (the root's fore plane --- the nose tip --- at the
world origin, \zfwd); a body-frame integrator passes the live attitude instead.
