...
($\zfwd$, fore-plane origin, see \cref{sec:philosophy}) the root is visited at the identity
pose, so its fore plane---the tip---sits at world $z = 0$ and its aft plane (the base rim) is at
world $z = -0.30$. The whole rocket extends aft into $-z$ from there. Two geometry-derived
landmarks anchor everything downstream:

\begin{itemize}
  \item the tip station, $\{z = 0,\ r_{\text{outer}} \approx 0\}$, and the aft station,
        $\{z = -0.30,\ r_{\text{outer}} = 0.0395\}$;
  \item the closed-form outer skin, $\text{radiusOuterAt}(z) = 0.0395\,(-z/0.30)$, which
...
The cone lives in the same local frame as every other part (\cref{sec:philosophy-oneframe}):
tip at $z=0$, base at $z=-L$. Its centroidal tensor (axisymmetric, unchanged) is paired with a CM
reported in that same frame, once the cone's formerly reversed \code{coneCmOffset}
(\src{model/parts/ConicalNoseCone.cpp}{49}) is corrected. The solid cone's CM lies $\bar h = L/4$
forward of the wide base, i.e.\ at $\code{cmLocalZ} = \bar h - L = -0.225$ (world $z = -0.225$, or
$3L/4$ aft of the tip), where the mass actually concentrates. It is derived by the same uniform rule
as every part, $-L/2 + \code{getCenterMassOffset()}.z()$ (\cref{sec:datamodel}), with no per-part
adapter; this is why CM falls out as a derived quantity rather than being authored, and why it comes
back relative to the recognizable nose tip rather than to the cone's own CM.

\tikzfig{worked-example}{The resolved \texttt{xl75\_multi} stack in the $\zfwd$ root frame (nose tip
at the origin). The nose occupies $z\in[-0.30,0]$; the body hangs aft over $z\in[-1.20,-0.30]$; the
fin set seats on the body wall near the aft end; the coupler nests into the body fore bore and
projects $0.04\,$m forward of the bore mouth, where its outer radius exceeds the tapering nose
skin.}{fig:worked}
...
$p = \text{nose}.\text{stationAt}(0) = \{z=-0.30,\ r_{\text{outer}}=0.0395\}$ (nose aft) and
$c = \text{body}.\text{stationAt}(1) = \{z=0,\ r_{\text{outer}}=0.0395\}$ (body fore), giving a child
fore-plane origin of $z = -0.30 + 0 - 0 = -0.30$. The body therefore spans world
$z \in [-1.20, -0.30]$, extending \emph{aft} ($-z$) from the nose base---physically correct, the
airframe hangs below the nose. The Layer-1 seam check is clean: $|0.0395 - 0.0395| = 0$.
...
$\text{gap}=0$. The resolver reads $p = \text{body}.\text{stationAt}(0.06)$, i.e. body-local
$z = (0.06-1)\cdot 0.90 = -0.846$, world $z \approx -1.146$, with $r_{\text{outer}}=0.0395$, and
seats the fin root's aft end near the body aft (world $\approx -1.20$). Layer~1 confirms radial compatibility: the fin's $\text{bodyRadius}$
...
bore). The resolver reads $p = \text{body}.\text{stationAt}(1.0)$ at body-local $z = 0$ (the
fore bore mouth, world $z=-0.30$) and $c = \text{coupler}.\text{stationAt}(0)$ at coupler-local
$z = -0.08$ (its aft plane), giving a child fore-plane origin of $0 + (-0.04) - (-0.08) = +0.04$ in
the body frame, which composes with the body's world fore origin $-0.30$ to world $z = -0.26$. The
coupler thus spans world $z \in [-0.34, -0.26]$: its aft $0.04\,$m (down to $-0.34$) is correctly
buried in the bore (mouth at $-0.30$), but its fore $0.04\,$m (up to $-0.26$) projects \emph{forward,
past the body fore rim, into the nose region}, because the $0.08\,$m coupler is longer than its
$0.04\,$m insertion depth. Layer~1 passes---coupler outer radius $0.0376 \le$ body inner radius
$0.0376$, so it fits the bore radially---which is precisely why a radial seam check alone is
insufficient and the envelope sweep of \cref{sec:diagnostics} is required.

\paragraph{(4) The forward poke-through, prevented.}
The coupler's forward $0.04\,$m occupies world $z \in [-0.30, -0.26]$, forward of the body fore
rim and inside the nose's axial span. The Layer-2 envelope sweep resolves the host by the
sorted-interval query at each feature breakpoint:

\begin{itemize}
  \item Over $z \in [-0.34, -0.30]$ the host is the body bore, with capacity
        $r_{\text{inner}} = 0.0376$; the coupler's $0.0376 \le 0.0376$ holds, so this span is
        clean.
  \item Over $z \in [-0.30, -0.26]$ the body is gone and the only covering envelope is the solid
        nose cone. The solid-host rule compares the coupler outer radius against the cone
        \emph{skin}: at the rim $\text{radiusOuterAt}(-0.30) = 0.0395\,(0.30/0.30) = 0.0395$
        (the coupler fits), but at the coupler fore, the worst station $z = -0.26$,
        $\text{radiusOuterAt}(-0.26) = 0.0395\,(0.26/0.30) = 0.0342$, while the coupler
...
The sweep emits \cppinline|OverlapDiagnostic{offender = coupler, host = nose, zWorld = -0.26,|
\cppinline|penetration = 0.0034}| with the message ``coupler OD $0.0376$ exceeds nose skin
radius $0.0342$ at $z=-0.26$, $0.04\,$m forward of the body rim (poke-through $0.0034\,$m)'', and sets
...
Body origin $z=-0.30$; spans $[-1.20,-0.30]$; seam $|0.0395-0.0395|=0$ \quad\ok \\
\addlinespace
Fins $\to$ body wall &
\seat{OnSurface}; $0.06 \!\to\! 0$; gap $0$ &
Root seats at world $z\approx-1.146$; $0.0395=0.0395$ \quad\ok \\
\addlinespace
Coupler $\to$ body bore &
\seat{NestInBore}; $1 \!\to\! 0$; gap $0.04$ &
Origin $z=-0.26$; spans $[-0.34,-0.26]$; OD $0.0376\le$ ID $0.0376$ \quad\ok (Layer~1) \\
\addlinespace
Coupler vs.\ nose skin &
(Layer-2 sweep over $[-0.30,-0.26]$) &
At $z=-0.26$, OD $0.0376 > $ skin $0.0342$; penetration $0.0034$ \quad\fail \\
\bottomrule
\end{tabularx}
\end{table}
