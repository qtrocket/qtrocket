...
(origin on the longitudinal axis at the part's fore plane, \zfwd) into the shared root frame. A
...
root and the pose at which to plant it (conventionally identity --- the root's fore plane, the
nose tip, at the world origin, \zfwd, the rocket extending into $-z$) and returns one \code{Placed}
per part.}\label{lst:resolver}
\end{listing}
...
// Child's fore-plane origin along the parent axis: line the two stations up,
...
\end{minted}
\caption{The per-child placement step, in \zfwd. Pure geometry: the child's fore-plane origin is
...
because the pose locates the child's \emph{fore plane} --- its local origin --- not the chosen
station, which sits \cppinline|c.z| from it); and the seat-signed gap
...
const double  cmLocalZ     = -child.getLength() / 2.0
                           + child.getCenterMassOffset().z();   // mid-plane (-L/2) + mid-referenced offset
const Vector3 childCmInRoot =
     childPose.origin
   + childPose.orient * Vector3(0.0, 0.0, cmLocalZ);
...
\end{minted}
\caption{The CM derivation. The local CM station is $-L/2 + \code{getCenterMassOffset()}.z()$,
evaluated identically for every part in the one shared \zfwd{} frame (fore-plane origin)
(\srccap{model/parts/Part.h}{96}). There is no per-type adapter and no cone special
case.}\label{lst:cm-derive}
\end{listing}

This is uniform precisely because the design admits no special parts (\cref{sec:philosophy-oneframe}).
The cone once reported its CM in a reversed internal frame (\code{coneCmOffset} at
\src{model/parts/ConicalNoseCone.cpp}{49}, base at $+z$); that reversal is a defect this design
\emph{removes}, correcting the helper so the cone reports in the same frame as every other part. With
no reversed frame left, no vector arithmetic crosses a frame boundary unwatched --- the same single
expression derives the CM of the cone, the tubes, the sphere, and the fin set alike.

Because the resolver plants the root at its fore origin --- the nose tip --- at the world origin, every
resolved station, and therefore \code{getCompositeCm()}, is expressed relative to the \textbf{nose tip}:
a fixed, recognizable geometric datum, not the root's own center of mass. For a conventional nose-led
rocket the whole-body CG comes back as $(0, 0, -d)$ with $d > 0$ --- the CG sits $d$ metres aft of the
tip. (The migration's invariance gate accounts for this deliberate datum change; see \cref{sec:migration}.)
