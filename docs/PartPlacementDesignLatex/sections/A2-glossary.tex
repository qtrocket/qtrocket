...
\item[Cone CM-frame adapter] The single explicitly stated transform that reconciles the
  cone's internal CM frame with the resolver's frame. \code{coneCmOffset}
  (\src{model/parts/ConicalNoseCone.cpp}{49}) expresses the cone CM in a
  mid-length-origin, base-at-$+z$ frame, whose $+z$ is the opposite of the chosen forward.
  The resolver instead works aft-plane-origin, \zfwd, where the forward-frame CM coordinate
  is $\mathit{cmZ}_{\mathrm{fwd}} = \bar h$ ($L/4$ solid, $L/3$ shell --- the CM sits near the
  wide base). The cone is the only part whose \code{cmStationLocal} override bridges a frame flip.

\item[\code{cmStationLocal}] A new per-type \code{Part} virtual returning a part's own CM
  as a single scalar axial coordinate in its \zfwd\ local frame. Symmetric parts return
  $L/2$; the cone overrides it with the adapter above. It is the sole consumer of
  \code{getCenterMassOffset}, so no implicit cross-frame vector addition survives.
...
\item[\code{getCenterMassOffset}] The accessor returning a part's CM relative to the
  ...
  for a cone. It is documented but unconsumed today; this design finally consumes it,
  exclusively inside \code{cmStationLocal}.
...
\item[\code{Pose}] A new value type carrying an origin (the part-local aft-plane origin in
...
\item[Station / fractional station (\code{station01})] A position along a part's
  ...
  $\code{station01}\times\code{getLength()}$, recomputed on each read so that editing a
...
\item[$+z = $ forward convention (\zfwd)] The settled longitudinal convention: $+z$ points
  toward the nose tip, every part's local origin sits on the axis at its aft plane, and a
  part occupies $z \in [0, +\code{length}]$. It matches the visualizer and the existing
  \code{.qrd} files, so the renderer change is a consumer swap and the file migration a
  re-expression, both with no axial sign flip. \zaft\ was rejected for requiring exactly
  those inversions.
