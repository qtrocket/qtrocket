...
\src{model/parts/Part.h}{326}
  & Storage member \code{childParts} changes type from
    ...
    \code{radiusOuterAt}, \code{radiusInnerAt}, \code{axialLength}, \code{isSolid}, and the
    promoted \code{getLength()} to the base class (none exist today; the only base geometry virtual
...
\srcf{model/parts/Part.cpp}
  & \code{computeCompositeAt} (\src{model/parts/Part.cpp}{169}) is rewritten to consume the
    resolver's \code{Pose} and derive CM through the uniform $-L/2 + \code{getCenterMassOffset()}.z()$
    rule, preserving the two-pass mass-weighted CM and parallel-axis structure (\cref{sec:resolver}).
...
\srcf{model/parts/ConicalNoseCone.cpp}
  & Adds \code{radiusOuterAt} (the linear taper $\text{baseRadius}\cdot(1-z/L)$) and
    \code{isSolid}. \emph{Corrects} the reversed-frame defect in \code{coneCmOffset}
    (\src{model/parts/ConicalNoseCone.cpp}{49}) so the CM is reported in the shared \zfwd{} frame
    ($L/2 - \bar h \rightarrow \bar h - L/2$); the centroidal tensor is unchanged
    (\cref{sec:philosophy-oneframe}). \\
\addlinespace
...
\srcf{model/parts/HollowSphere.h}, \srcf{model/parts/HollowSphere.cpp}
  & Each gains \code{radiusOuterAt} / \code{radiusInnerAt} and \code{isSolid} backed by its
    ...
    \code{FinSet::finSetCmOffset} (\src{model/parts/FinSet.cpp}{52}) is also \emph{corrected} to
    report its CM relative to mid-length ($x_c \rightarrow x_c - L/2$), so the uniform CM rule
    applies (\cref{sec:datamodel}). \\
\addlinespace
...
\addlinespace
\srcf{model/tests/NoseConeTests.cpp}, \srcf{model/tests/FinSetTests.cpp},
\srcf{model/tests/AeroTests.cpp}
  & Updated to the corrected CM references: the cone CM-offset assertions flip sign
    ($L/2 - \bar h \rightarrow \bar h - L/2$, e.g.\ solid $+L/4 \rightarrow -L/4$); the FinSet CM
    assertion (\code{off.z() == $x_c$}) becomes \code{off.z() == $x_c - L/2$}; and the aero $x_{cp}$
    expectations that read \code{getCenterMassOffset} are adjusted to match. These tests were written
    around the old references; correcting the defects requires correcting them
    (\cref{sec:philosophy-oneframe}). \\
\bottomrule
\end{tabularx}
\end{table}
