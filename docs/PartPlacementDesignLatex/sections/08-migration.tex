...
One datum detail is controlled so the comparison is genuinely apples-to-apples.
\code{getCompositeMass} and the composite inertia \emph{about the CM} are datum-independent and are
compared bit-for-bit directly. \code{getCompositeCm}, however, is datum-relative, and this design
\emph{deliberately changes its datum}: the incumbent reports the CG relative to the root part's
\emph{own} CM (\src{model/parts/Part.cpp}{169}), whereas the new model reports it relative to the
root's \emph{fore plane} --- the nose tip (\cref{sec:philosophy,sec:resolver}). That is the intended
improvement (a recognizable geometric datum), so the gate does \emph{not} expect a bit-identical
\code{getCompositeCm}; it compares the CG after re-expressing the legacy snapshot into the tip datum by
the one fixed, known offset between the two --- the root's own CM station,
$\code{cmLocalZ}_{\text{root}} = -L/2 + \code{getCenterMassOffset()}.z()$ (e.g.\ $-0.225$ for a solid
$L=0.30$ nose). The shim guarantees the per-part CM-to-CM \emph{differences} match (\cref{lst:shim}),
so once that single datum shift is applied the CG matches as well.
...
\code{axialLength}, \code{isSolid}---are added to the base \code{Part} and overridden on
the four concrete part types, and the promoted \code{getLength()} virtual joins them. The
same phase corrects the \emph{two} latent CM-reference defects so the uniform CM rule
(\cref{sec:datamodel}) holds with no special parts: \code{ConicalNoseCone::coneCmOffset}
(\src{model/parts/ConicalNoseCone.cpp}{49}) is fixed to report the CM in the shared \zfwd{} frame
($L/2 - \bar h \rightarrow \bar h - L/2$), and \code{FinSet::finSetCmOffset}
(\src{model/parts/FinSet.cpp}{52}) is fixed to report relative to mid-length ($x_c \rightarrow
x_c - L/2$). Both centroidal tensors are unchanged. The tests written around the old references
---\srcf{model/tests/NoseConeTests.cpp}, \srcf{model/tests/FinSetTests.cpp}, and the aero $x_{cp}$
checks in \srcf{model/tests/AeroTests.cpp}---are updated in lock-step. The single child-storage
...
resolver, by contrast, places a child by its \emph{fore-plane origin} in the parent
...
//
// 1. Recover the child's fore-plane origin in the parent frame. *CmLocalZ is the
//    uniform local CM station, -L/2 + getCenterMassOffset().z(), in the shared
//    +z-forward frame -- the same expression the forward derivation uses, so it
//    inverts cleanly with no per-part special case.
...
\code{StationLink} from a legacy CM-to-CM \code{Vector3}. It uses the same uniform local-CM
expression as the forward derivation (\cref{sec:datamodel}), so the recovery inverts it exactly
for every part, the cone included.}\label{lst:shim}
\end{listing}
...
fixture here. The cone's formerly reversed \emph{internal} CM frame
(\code{coneCmOffset}, \src{model/parts/ConicalNoseCone.cpp}{49}) is corrected in this same
phase so that it, too, reports in the shared forward frame
(\cref{sec:philosophy-oneframe}); there is no adapter to absorb a discrepancy and no link to
mirror.
\end{keyinsight}
...
\code{StationLink}s---and asserts \emph{bit-identical} \cppinline|getCompositeMass(0)| and
\cppinline|getCompositeI(0)| (both datum-independent) against the Phase-0 snapshots, plus
\cppinline|getCompositeCm(0)| and the resolved stations after applying the single known
datum shift (the root's own CM station; see the Phase-0 datum note). Equality is exact, not
within a tolerance: the migration is a re-expression of the same arithmetic over the same
geometry, so any deviation is a defect, and a tolerance would merely hide it.

A dedicated sub-test pins the nose CM to a hand-computed value as the specific guard
that the cone-frame correction (\cref{sec:philosophy-oneframe}) landed correctly. For the
\code{xl75\_multi} nose---a solid cone of length \cppinline|L = 0.30|
(\src{tests/data/designs/xl75\_multi.qrd}{5})---the centroid sits at $\bar{h} = L/4$ forward of
the base, so its local CM station is $-L/2 + \code{getCenterMassOffset()}.z() = -L/2 + (\bar{h} -
L/2) = \bar{h} - L = -0.225$ (\cref{sec:datamodel}): $3L/4$ aft of the tip. Pinning this one scalar
to a value computed by hand---independently of both the legacy and migrated code paths---is the test
that catches a sign error in the corrected cone CM before it can propagate.

\begin{keyinsight}[Why the cone gets its own hand-checked sub-test]
A symmetric part has its CM at mid-length, so its local CM station is exactly $-L/2$ and a sign
slip cannot hide there. The cone is the only current part whose CM is off-center \emph{and} whose
helper formerly carried a reversed sign, so it is the one part for which a stale-sign mistake
could survive the aggregate check yet still be wrong. The hand-computed
\cppinline|-0.225| isolates exactly that risk.
\end{keyinsight}
