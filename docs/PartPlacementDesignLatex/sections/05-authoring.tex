...
reconstructs the equivalent \code{StationLink} from the legacy CM-to-CM offset using the
explicit cone frame adapter, the sign-correct derivation of which is the subject of
...
to a fore-to-fore $\{1, 1, depth, \seat{NestInBore}\}$ insertion, and \code{seatOnWall} to
a wall station with the child's aft plane on the surface. They add no expressive power over
